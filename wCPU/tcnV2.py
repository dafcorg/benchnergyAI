# train_convnext_monitor_ddp.py
# Script completo: ConvNeXt + ImageNet (ImageFolder) + DDP multi-GPU + monitorización (GPU + CPU nivel A + estado algoritmo)
# Salidas: CSV por rank, métricas por época, gráficas, modelo, config.json, experiment_summary.json, e integración opcional de Nsight Compute (ncu CSV)
...
CONFIG_DEFAULT = {
    "data_dir": "./imagenet1k",              # Debe contener train/ y val/ (ImageFolder)
    "log_dir": "./logs_convnext",
    "batch_size": 128,                       # IMPORTANTE: batch_size por GPU 
    "num_workers": 8,
    "num_epochs": 20,
    "lr": 1e-3,
    "weight_decay": 0.05,
    ...
    "backend": "nccl",                       # DDP backend: "nccl" (GPU) o "gloo"
...
def parse_args() -> Dict[str, Any]:
    parser = argparse.ArgumentParser(description="ConvNeXt + monitorización + DDP multi-GPU + tracking JSON")
...
def set_seed(seed: int):
    ...
    # Nota: determinism puede afectar rendimiento; pero es útil para reproducibilidad.
...
class GPUMonitorThread(threading.Thread):
    def __init__(self, device_index: int, interval: float, enable_sm_metrics: bool):
        ...
        # Warm-up para que cpu_percent no devuelva 0 por falta de ventana previa
...
def get_model(num_classes: int) -> nn.Module:
    # ConvNeXt Tiny con pesos por defecto (preentrenado)
...
def ddp_reduce_mean(value: float, device: torch.device, use_ddp: bool) -> float:
    # Reduce (mean) de un escalar float a través de ranks
    if not use_ddp:
        return value
...
def predict_all_ddp(
    ...
    # En DDP cada rank predice su partición del loader (si usa DistributedSampler)
    # Para matriz de confusión global, juntamos todo en rank 0.
    ...
    # En NCCL, all_gather requiere tensores CUDA. Usamos device actual.
    ...
    # Relleno con -1 para poder all_gather con tamaños iguales
    pad_value = -1
...
def parse_ncu_csv(ncu_csv_path: Optional[str]) -> Optional[Dict[str, float]]:
    # Lee un CSV de ncu y extrae un resumen agregado.
    # Ajusta las claves si cambias el set de métricas en el comando ncu.
...
def main():
    ...
    # DDP activación automática si el script se lanzó con torchrun
    ...
    # Device final: 1 proceso por GPU (cuda:{LOCAL_RANK})
    ...
    # Semilla por rank (si se desea)
    seed = int(config["seed"])
    if use_ddp and bool(config.get("seed_offset_per_rank", True)):
        seed = seed + rank
    set_seed(seed)

    # Directorio del experimento: se crea solo en rank 0 y luego se sincroniza
    ...
    # Compartir exp_dir a todos los ranks
    exp_dir_list = ddp_all_gather_object(exp_dir, use_ddp)
    exp_dir = exp_dir_list[0]  # rank0 define el exp_dir real

    ddp_barrier(use_ddp)

    # Logs iniciales solo en rank0
    if ddp_is_main(rank, use_ddp):
    ...
    # DataLoaders (con DistributedSampler en DDP)
    ...
    # Monitor: cada rank monitoriza su propia GPU (device_index = local_rank)
    ...
    try:
        for epoch in range(1, int(config["num_epochs"]) + 1):
            # En DDP es obligatorio set_epoch para que el shuffling sea distinto por época
            ...
            # Reducir métricas por época a medias globales (comparables entre experimentos)
            ...
            # Energía total GPU (suma de ranks)
            if use_ddp:
    ...
    # Energía total acumulada (suma de ranks)
    if use_ddp and device.type == "cuda":
        e_t = torch.tensor([energy_J_rank], device=device, dtype=torch.float64)
        dist.all_reduce(e_t, op=dist.ReduceOp.SUM)
    ...
    # Exportar CSV de monitor por rank
    ...
    # Recolectar sm_profile de todos los ranks en rank0
    sm_profiles_all = ddp_all_gather_object(sm_profile_rank, use_ddp)

    # En rank0: guardar modelo, métricas, gráficas, confusion matrix, summary
    if ddp_is_main(rank, use_ddp):
        # Guardar modelo (si DDP, extraer module)
        ...
        # Predicciones globales para matriz de confusión y F1
        ...
        if y_true is None or y_pred is None:
            # En rank0 debería llegar algo, pero se protege por seguridad
        ...
        # Gráfica monitorización: aquí se grafica solo el rank0 (hay un PNG por experimento).
        # Para ver todas las GPUs, se recomienda graficar a partir de los CSV por rank en post-procesado.
        monitor_fig_rank0 = os.path.join(exp_dir, "monitoring_raw_rank0.png")

        # Leer el CSV rank0 para graficar valores crudos (evita depender del objeto en memoria)
        times = []
        power = []
        util_gpu = []
        mem_used = []
        ...
        # Lista de CSVs por rank para trazabilidad
